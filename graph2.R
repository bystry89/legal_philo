...
#leid <- 
  #cluster_louvain(G2)
#  cluster_leiden(G2, objective_function = "modularity",resolution_parameter = 0.6, n_iterations = 100)
#sort(table(leid$membership))
#sum(table(leid$membership)[table(leid$membership)>70])/sum(table(leid$membership))

eigen <- eigen_centrality(G2, weights = E(G2)$weight)$vector

cmp_nodes <- 
  #data.frame(id=leid$names, `Modularity Class`=leid$membership) %>% 
  nodes_cmp %>% left_join(data.frame(id=names(eigen), eigen=as.numeric(eigen))) 
#%>% rename(Modularity.Class = modularity_class)
cmp_nodes$eigen <- eigen
