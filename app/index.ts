...
renderer.setSetting("nodeReducer", (node, data) => {
  ...
  // Scale down node size by a factor of 0.5
  res.size = data.size * 0.5;

  if (state.hoveredNode === node) {
    // Concatenate 'label' and 'display_name' for the hovered node
    const displayName = graph.getNodeAttribute(node, "display_name") as string;
    res.label = `${data.label} (${displayName})`;
  } else if (state.hoveredNeighbors && !state.hoveredNeighbors.has(node)) {
    res.label = "";
    res.color = "#f6f6f6";
  }
