...
nodes %>% 
  ...
  group_by(cluster_label) %>% 
  summarize(US = mean(country == "US", na.rm=T),
            other_English = mean(country %in% c("GB", "CA", "AU", "NZ", "IE"), na.rm=T),
            non_English = mean(!country %in% c("GB", "CA", "AU", "NZ", "IE", "US"), na.rm=T)) %>% 
  mutate_at(c("US", "other_English", "non_English"), round, 2) %>% 
 DT::datatable(
...
nodes %>%
  ...
  filter(!is.na(type)) %>%
  group_by(type) %>%
  summarize(n = n(), mean_eigen = round(mean(eigencentrality, na.rm=T), 3)) %>%
  filter(n>20) %>%
...
nodes %>%
  ...
  filter(type %in% c("book", "journal-article", "article")) %>%
  ggplot(aes(x=publication_year, y=eigencentrality, color= type)) +
...
nodes %>%
  ...
  lm(eigencentrality ~ type*publication_year, data = .) %>%
  broom::tidy() %>%
...
read_csv("../data/cits/out_by_clust.csv") %>% 
